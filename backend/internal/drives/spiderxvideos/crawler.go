...
// 默认 author/tag 标签，便于在前端筛选 XVideos 来源的视频。
const DefaultAuthor = "xvideos"
const DefaultTag = "xvideos"

// DefaultTargetNew 是凌晨任务默认的"凑够这么多新视频"目标数。
const DefaultTargetNew = 15

const DefaultStartURL = "https://www.xvideos.com/"

// 视频下载、列表页请求的 UA 沿用爬虫脚本里那一套。
...
type CrawlerConfig struct {
	...
	ProxyURL string
	// StartURL 是传给 Python 的 XVideos 起始列表 / 搜索 URL。
	StartURL string
	// Keyword 是传给 Python 的搜索关键词；StartURL 为空时才会生效。
	Keyword string
	// Quality 是传给 Python 的源选择策略：best/hd/high/low/hls。
	Quality string
	// MinSize / MaxSize 是传给 Python 的文件大小过滤条件。
	MinSize string
	MaxSize string
	// MinDuration / MaxDuration 是传给 Python 的时长过滤条件。
	MinDuration string
	MaxDuration string
	// MergeHLS 控制 Python/Go 是否启用 HLS 合并路径。
	MergeHLS bool
	// FFmpegPath 是 MergeHLS 使用的 ffmpeg 可执行文件路径。
	FFmpegPath string
	// Cookie 可选，遇到站点侧限制时透传给 Python 和下载请求。
	Cookie string
...
func NewCrawler(cfg CrawlerConfig) *Crawler {
	...
	if strings.TrimSpace(cfg.StartURL) == "" && strings.TrimSpace(cfg.Keyword) == "" {
...
func (c *Crawler) writeSeenViewkeys(ctx context.Context, path string) (int, error) {
	seenIDs, err := c.cfg.Catalog.ListVideoIDSuffixesByPrefix(ctx, Kind, c.cfg.Driver.ID())
	if err != nil {
		return 0, err
	}
...
func (c *Crawler) startSpiderTargetNew(ctx context.Context, targetNew int, seenPath, outputPath string) (*exec.Cmd, io.ReadCloser, error) {
	args := []string{
		...
		"--seen-file", seenPath,
		"--output", outputPath,
		"--keyword", strings.TrimSpace(c.cfg.Keyword),
		"--quality", strings.TrimSpace(c.cfg.Quality),
		"--min-size", strings.TrimSpace(c.cfg.MinSize),
		"--max-size", strings.TrimSpace(c.cfg.MaxSize),
		"--min-duration", strings.TrimSpace(c.cfg.MinDuration),
		"--max-duration", strings.TrimSpace(c.cfg.MaxDuration),
		"--no-download",
		"--quiet",
		"--stream-output",
	}
	if strings.TrimSpace(c.cfg.StartURL) != "" {
		args = append(args, "--url", strings.TrimSpace(c.cfg.StartURL))
	}
	if c.cfg.MergeHLS {
		args = append(args, "--merge-hls")
	}
	if strings.TrimSpace(c.cfg.Cookie) != "" {
		args = append(args, "--cookie", strings.TrimSpace(c.cfg.Cookie))
	}
	if strings.TrimSpace(c.cfg.ProxyURL) != "" {
		args = append(args, "--proxy", strings.TrimSpace(c.cfg.ProxyURL))
	}
...
func (c *Crawler) processOne(ctx context.Context, videoID string, item spiderVideoEntry) error {
	...
	if viewkey == "" {
		viewkey = strings.TrimSpace(item.VideoID)
	}
	sourceID := sourceIDForItem(item)
	if sourceID == "" {
	...
	// 视频先下载（必须）；HLS 可选走 ffmpeg 合并路径。
	var videoSize int64
	var videoErr error
	if isStreamingManifest(videoURL) {
		if !c.cfg.MergeHLS {
			return fmt.Errorf("streaming manifest is not supported for direct download: %s", videoURL)
		}
		videoSize, videoErr = c.downloadHLSMerged(ctx, item, videoPath, sourceID)
	} else {
		videoSize, videoErr = c.downloadVideoAtomicWithRefresh(ctx, item, videoPath, videoURL, sourceID)
	}
	if videoErr != nil {
...
func (c *Crawler) downloadAtomic(ctx context.Context, src, dst, referer string) (int64, error) {
	...
	if strings.TrimSpace(c.cfg.Cookie) != "" {
		req.Header.Set("Cookie", strings.TrimSpace(c.cfg.Cookie))
	}
...
func (c *Crawler) downloadHLSMerged(ctx context.Context, item spiderVideoEntry, dst, sourceID string) (int64, error) {
	videoURL := strings.TrimSpace(item.VideoURL)
	if videoURL == "" {
		return 0, errors.New("empty url")
	}
	...
	if err != nil {
		_ = os.Remove(tmp)
		return 0, fmt.Errorf("ffmpeg hls merge: %w: %s", err, strings.TrimSpace(string(out)))
	}
	...
	log.Printf("[spider91] drive=%s source_id=%s merged hls via ffmpeg", c.cfg.Driver.ID(), sourceID)
...
func (c *Crawler) resolveFreshVideoURL(ctx context.Context, item spiderVideoEntry) (string, error) {
	...
	cookieHeader := strings.TrimSpace(c.cfg.Cookie)
...
var (
	...
	xvideosURLRE = regexp.MustCompile(`(?:html5player\.)?setVideoUrl(?:HD|High|Low)\(\s*['"]((?:\\.|[^'"\\])+)['"]\s*\)`)
)
...
func parseSpider91VideoURL(html string) string {
	...
	if matches := xvideosURLRE.FindAllStringSubmatch(html, -1); len(matches) > 0 {
		return decodeXVideosJSString(matches[0][1])
	}
...
func decodeXVideosJSString(raw string) string {
	repl := strings.NewReplacer(`\/`, `/`, `\u0026`, `&`, `\u003d`, `=`, `\?`, `?`)
	return repl.Replace(strings.TrimSpace(raw))
}
...
func sourceIDForItem(item spiderVideoEntry) string {
	if id := sanitizeSourceID(item.VideoID); isValidSourceID(id) {
		return id
	}
	if id := sanitizeSourceID(item.Viewkey); isValidSourceID(id) {
		return id
	}
	if id := sanitizeSourceID(item.SourceID); isValidSourceID(id) {
		return id
	}
...
func isStreamingManifest(rawURL string) bool {
	u, err := url.Parse(strings.TrimSpace(rawURL))
	if err != nil || u == nil {
		return false
	}
